...
export function MetricCard({
  ...
  const displayValue =
    typeof value === "number" && animate ? animatedValue.toLocaleString() : value;
    ...
    <div className="metric-card animate-slide-up group hover:shadow-accent-glow/5 transition-shadow duration-300">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 sm:gap-3">
          <div
            className={`p-1.5 sm:p-2 rounded-lg ${color} transition-transform group-hover:scale-110 duration-300`}
          >
        ...
        {trend && (
          <span
            className={`flex items-center gap-1 text-xs font-medium ${trend.value >= 0 ? "text-emerald-400" : "text-red-400"}`}
          >
            {trend.value >= 0 ? (
