...
const COLORS = {
  ...
  info: { bg: "bg-accent-dynamic/10", border: "border-accent-dynamic/20", text: "text-accent-dynamic" },
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-theme flex items-center gap-2">
            ...
            {unreadCount > 0 && (
              <span className="px-2 py-0.5 text-xs bg-accent-dynamic/20 text-accent-dynamic rounded-full">{unreadCount}</span>
            )}
          </h1>
        ...
        <div className="flex items-center gap-3">
          <div className="flex gap-1 p-1 bg-theme-surface-2 rounded-xl border border-white/5">
            <button onClick={() => setFilter("all")} className={`px-3 py-1 text-xs rounded-lg ${filter === "all" ? "bg-accent-dynamic/20 text-accent-dynamic" : "text-theme-tertiary"}`}>All</button>
            <button onClick={() => setFilter("unread")} className={`px-3 py-1 text-xs rounded-lg ${filter === "unread" ? "bg-accent-dynamic/20 text-accent-dynamic" : "text-theme-tertiary"}`}>Unread</button>
          </div>
          {unreadCount > 0 && (
            <button onClick={markAllRead} className="text-xs text-accent-dynamic hover:underline">Mark all read</button>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        {displayed.length > 0 ? displayed.map((notif) => {
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className={`text-sm font-medium ${notif.read ? "text-theme-secondary" : "text-theme"}`}>{notif.title}</p>
                  {!notif.read && <span className="w-2 h-2 rounded-full bg-accent-dynamic flex-shrink-0" />}
                </div>
          ...
          <div className="glass-card p-12 text-center">
            ...
            <p className="text-theme-secondary">No {filter === "unread" ? "unread " : ""}notifications</p>
          </div>
        )}
      </div>
    </div>
