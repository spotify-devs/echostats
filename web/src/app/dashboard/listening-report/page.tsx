...
import { FileText, Clock, Music, Users, Disc3, TrendingUp, TrendingDown, Minus } from "lucide-react";
...
export default function ListeningReportPage() {
  ...
  function TrendBadge({ current, previous }: { current: number; previous: number }) {
    ...
    if (diff > 0) return <span className="inline-flex items-center gap-0.5 text-[10px] text-emerald-400"><TrendingUp className="w-3 h-3" />+{diff}%</span>;
    if (diff < 0) return <span className="inline-flex items-center gap-0.5 text-[10px] text-red-400"><TrendingDown className="w-3 h-3" />{diff}%</span>;
    return <span className="inline-flex items-center gap-0.5 text-[10px] text-theme-tertiary"><Minus className="w-3 h-3" />0%</span>;
  }

  const genrePie = (monthData?.top_genres || []).slice(0, 6).map((g: any) => ({ name: g.name, value: g.play_count }));
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {isLoading ? (
        <div className="space-y-6"><CardSkeleton /><ChartSkeleton /><ChartSkeleton /></div>
      ) : (
        <>
          {/* Summary Stats */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { icon: Clock, label: "Hours", value: monthData?.total_hours || 0, prev: weekData?.total_hours || 0, color: "text-accent-purple" },
              { icon: Music, label: "Tracks", value: monthData?.total_tracks_played || 0, prev: weekData?.total_tracks_played || 0, color: "text-spotify-green" },
              { icon: Users, label: "Artists", value: monthData?.unique_artists || 0, prev: weekData?.unique_artists || 0, color: "text-accent-cyan" },
              { icon: Disc3, label: "Genres", value: monthData?.unique_genres || 0, prev: weekData?.unique_genres || 0, color: "text-accent-pink" },
            ].map((stat) => (
              ...
              <div className="space-y-3">
                {(monthData?.top_artists || []).slice(0, 5).map((a: any, i: number) => (
                  <div key={i} className="flex items-center gap-3">
                    <span className="w-6 text-center text-sm font-bold text-accent-dynamic">{a.rank}</span>
                    {a.image_url && (
                      <div className="relative w-10 h-10 rounded-full overflow-hidden bg-theme-surface-3">
                        <Image src={a.image_url} alt="" fill className="object-cover" sizes="40px" />
                      </div>
                    )}
