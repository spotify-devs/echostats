  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        {isLoading ? <ListSkeleton rows={5} /> : (data?.items || []).length > 0 ? (
            ...
            {data.items.map((album: any) => (
              <div key={album.id} className="glass-card-hover p-3 space-y-2 group">
                <div className="relative aspect-square rounded-xl overflow-hidden bg-theme-surface-3">
                  {album.image ? (
                    <Image src={album.image} alt="" fill className="object-cover group-hover:scale-105 transition-transform" sizes="200px" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center"><Music className="w-8 h-8 text-theme-tertiary" /></div>
                  )}
                </div>
          ...
          <div className="glass-card p-8 text-center text-theme-tertiary">Connect Spotify to see saved albums</div>
        )}
      </section>
    </div>
  );
