      ...
      {isLoading ? (
        ...
        <>
              ...
              {[
                {
                  icon: Clock,
                  label: "Hours",
                  value: thisWeek?.total_hours || 0,
                  prev: (lastMonth?.total_hours || 0) / 4,
                  color: "text-accent-purple",
                },
                {
                  icon: Music,
                  label: "Tracks",
                  value: thisWeek?.total_tracks_played || 0,
                  prev: (lastMonth?.total_tracks_played || 0) / 4,
                  color: "text-spotify-green",
                },
                {
                  icon: Users,
                  label: "Artists",
                  value: thisWeek?.unique_artists || 0,
                  prev: lastMonth?.unique_artists || 0,
                  color: "text-accent-cyan",
                },
                {
                  icon: Flame,
                  label: "Streak",
                  value: thisWeek?.listening_streak_days || 0,
                  prev: 0,
            ...
            {topTrack && (
              <div className="glass-card p-5 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-theme truncate max-w-[180px]">
              ...
              <div className="space-y-2">
                {(thisWeek?.top_tracks || []).slice(0, 5).map((t: any, i: number) => (
                  <div key={i} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="flex-1 text-theme-secondary truncate">
                      {t.name?.split(" — ")[0]}
                    </span>
          ...
          <div className="text-center py-4">
            <Link
              href="/dashboard"
              className="text-sm text-accent-dynamic hover:underline inline-flex items-center gap-1"
            >
