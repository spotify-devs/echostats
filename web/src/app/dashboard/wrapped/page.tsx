...
import { Gift, Share2, Download, Music, Users, Clock, Disc3, Flame } from "lucide-react";
...
export default function WrappedPage() {
  ...
  if (isLoading) return <div className="space-y-6 max-w-lg mx-auto"><ChartSkeleton height={600} /></div>;
  ...
  return (
    <div className="space-y-6 max-w-lg mx-auto pb-12">
      ...
      {/* The Card */}
      <div id="wrapped-card" className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-surface-1 via-surface to-surface-2 border border-white/10 shadow-2xl">
        {/* Header */}
        <div className="p-8 pb-4 text-center bg-gradient-to-b from-accent-dynamic/10 to-transparent">
        ...
        <div className="grid grid-cols-2 gap-4 px-8 pb-6">
          <div className="p-4 rounded-2xl bg-white/[0.03] text-center">
            ...
            <p className="text-xl font-bold text-theme">{(data?.total_tracks_played || 0).toLocaleString()}</p>
            ...
            <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.03]">
              {topArtist.image_url && (
                <div className="relative w-12 h-12 rounded-full overflow-hidden flex-shrink-0">
                  <Image src={topArtist.image_url} alt="" fill className="object-cover" sizes="48px" />
                </div>
              )}
              <div className="flex-1 min-w-0">
                <p className="text-[10px] text-accent-dynamic uppercase tracking-wider">#1 Artist</p>
            ...
            <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.03]">
              {topTrack.image_url && (
                <div className="relative w-12 h-12 rounded-xl overflow-hidden flex-shrink-0">
                  <Image src={topTrack.image_url} alt="" fill className="object-cover" sizes="48px" />
                </div>
              )}
              <div className="flex-1 min-w-0">
                <p className="text-[10px] text-accent-dynamic uppercase tracking-wider">#1 Track</p>
                <p className="text-sm font-bold text-theme truncate">{topTrack.name?.split(" — ")[0]}</p>
              </div>
        ...
        <div className="px-8 pb-6 text-center">
          <p className="text-[10px] text-theme-tertiary">Generated by EchoStats · echostats.local</p>
        </div>
      </div>
